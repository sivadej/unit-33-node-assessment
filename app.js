...
app.post('/', async function (req, res) {
	...
	const { developers } = req.body; // should be array of names
	if (!developers) res.send('Invalid request received');

	// create an array of promises resolving API response per name
	let userData = await Promise.all(developers.map(username =>
